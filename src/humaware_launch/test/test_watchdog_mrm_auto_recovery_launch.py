"""End-to-end launch test for automatic recovery from a watchdog MRM.

The watchdog auto-trigger tests prove that a lost heartbeat *declares* an
MRM. This test proves the return trip: a watchdog-driven MRM clears
itself the moment its triggering condition is resolved, with **no**
``clear_mrm`` service call. A watchdog MRM is a live function of the
runtime's health, not a latched state -- unlike a manually triggered MRM
or an E-stop, which require an explicit release.

Using the teleop heartbeat watchdog (``teleop_heartbeat_timeout_triggers_mrm
:=true`` with a short timeout) the test:

* enters ``MODE_TELEOP`` and streams ``teleop/heartbeat`` +
  ``teleop/cmd_vel`` until teleop is approved while safety is healthy,
* stops the heartbeat (keeping the command) and asserts the watchdog
  declares ``STATE_MRM`` with ``teleop_heartbeat_timeout``,
* resumes the heartbeat and asserts safety leaves ``STATE_MRM`` and
  teleop is approved again under ``SOURCE_TELEOP`` -- purely from the
  heartbeat returning, with no service intervention.
"""
...
ROBOT_ID = f"mock_wd_recover_{os.getpid()}"
TELEOP_LINEAR = 0.10
TELEOP_ANGULAR = 0.04
MATCH_TOL = 1e-6
...
class TestWatchdogMrmAutoRecovery(unittest.TestCase):
    def test_watchdog_mrm_clears_when_heartbeat_returns(self, robot_id):
        with WaitForTopics(
        ...
        node = rclpy.create_node(
            f"wd_recover_launch_test_{os.getpid()}",
            context=context,
        )
        ...
        try:
            ...
            # --- Phase 1: healthy -> teleop approved -----------------------
            self._stream_until(
                executor=executor,
            ...
            # --- Phase 2: heartbeat stops -> watchdog MRM ------------------
            safety_states.clear()
            self._stream_until(
                executor=executor,
                ...
                check=lambda: self._watchdog_mrm_observed(safety_states),
                timeout_s=8.0,
                failure_message="teleop heartbeat loss did not declare a watchdog MRM",
            )

            # --- Phase 3: heartbeat returns -> auto recovery ---------------
            ...
            self._stream_until(
                ...
                timeout_s=8.0,
                failure_message=(
                    "watchdog MRM did not clear automatically when the heartbeat"
                    " returned"
                ),
            )
        finally:
    ...
    @staticmethod
    def _watchdog_mrm_observed(safety) -> bool:
        return any(
    ...
    def _mode_request(target_mode: int) -> SetMode.Request:
        ...
        request.requester = "wd_recover_launch_test"
        request.reason = "watchdog_recovery_e2e"
        return request
...
class TestWatchdogMrmAutoRecoveryShutdown(unittest.TestCase):
